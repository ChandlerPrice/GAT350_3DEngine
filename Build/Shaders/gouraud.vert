...
void main()
{
	...
	vec3 direcion_to_light = normalize(vec3(light.position - position));

	float lDotN = max(dot(direcion_to_light, normal), 0); //intensity
	...
	if(lDotN > 0)
	{
		vec3 direcion_to_view = normalize(-position.xyz);
		vec3 reflection = reflect(-direcion_to_light, normal);
